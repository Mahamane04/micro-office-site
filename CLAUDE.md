...
## ⚠️ Crédits Netlify (plan gratuit : 300/mois) — LES BUILDS COÛTENT CHER
Sur ce plan, ce sont les **builds** qui consomment les crédits (~13 crédits/build observé), PAS les visites. Règles :
- **Ne jamais rebuild pour du contenu.** Les pages sont en ISR → une modif Airtable apparaît toute seule (cache ~5 min) sans reconstruire. Le webhook `revalidate` est **purge-only** (pas de build) ; le Build Hook Netlify a été **supprimé**.
- **Grouper les changements de code** en un seul commit/push quand c'est possible — chaque push = 1 build = ~13 crédits.
- Éviter de déclencher des builds manuels (API `/builds`, redeploys) pour tester.
- Le pic du 23/07 (50% en <24h) = les ~11 builds du jour de mise en place/redesign + tests du build hook. Ponctuel, pas récurrent.
...
- **Webhook de revalidation** — `netlify/functions/revalidate.ts` (POST, header `x-webhook-secret` = `NETLIFY_ISR_SECRET`). Fait DEUX choses, car le site mélange deux modes de rendu :
  ...
  - Le webhook est **purge seulement** (aucune reconstruction). Optionnel : sert juste à forcer un rafraîchissement immédiat au lieu d'attendre l'expiration du cache ISR (~5 min).
